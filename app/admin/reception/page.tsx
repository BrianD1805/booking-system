...
export default function ReceptionBookingPage() {
  ...
  const [customerSearchMessage, setCustomerSearchMessage] = useState('Search for a client first, or switch to ad-hoc patient mode.');
  ...
  const canSave = Boolean(selectedTime && bookingPractitionerId && patientName.trim() && patientPhone.trim() && patientEmail.trim());
  ...
  async function handleCustomerSearch() {
    ...
    try {
      ...
      setCustomerSearchMessage(results.length ? `${results.length} matching client${results.length === 1 ? '' : 's'} found.` : 'No matching client found. You can use ad-hoc patient mode below.');
    } catch (error) {
    ...
    <main className="shell fresh-shell admin-shell reception-shell">
      ...
      <section className="admin-command reception-command">
        <div>
          ...
          <p className="hero-copy tight-copy">A full-page receptionist workflow with proper client search, clear diary slots and one final save button.</p>
        </div>
      ...
      <form className="reception-booking-page" onSubmit={handleSaveBooking}>
        <section className="card clean-panel reception-card reception-client-card">
          <div className="section-heading-row compact-row">
            <div>
              <p className="badge blue-badge">Step 1</p>
              <h2 className="section-title compact">Find client</h2>
              <p className="mini-copy">Search by name, phone or email before booking.</p>
            </div>
            <button className="pill" type="button" onClick={startAdhocPatient}>Ad-hoc patient</button>
          </div>

          <div className="reception-mode-tabs" aria-label="Reception booking mode">
            <button type="button" className={receptionMode === 'search' ? 'active' : ''} onClick={() => setReceptionMode('search')}>Search client</button>
            <button type="button" className={receptionMode === 'adhoc' ? 'active' : ''} onClick={startAdhocPatient}>Ad-hoc patient</button>
          </div>
          ...
          {selectedCustomer && (
            <article className="selected-client-card">
              <div>
              ...
              <button className="pill" type="button" onClick={clearSelectedClient}>Change</button>
            </article>
          )}

          {customerResults.length > 0 && !selectedCustomer && (
            <div className="customer-result-list reception-results">
          ...
          <div className="reception-details-grid">
        ...
        <section className="card clean-panel reception-card reception-slot-card">
          <div className="section-heading-row compact-row">
            <div>
          ...
          <div className="slot-grid reception-slot-grid">
        ...
        <section className="card clean-panel reception-card reception-confirm-card">
          <div>
            <p className="badge blue-badge">Step 3</p>
            <h2 className="section-title compact">Confirm</h2>
            <p className="mini-copy">Review the receptionist booking before saving it into the live diary.</p>
          </div>

          <div className="confirmation-card reception-summary-card">
